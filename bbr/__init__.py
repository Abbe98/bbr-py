...
The majority of items are buildings followed by facilities, persons, organizations, and then environments. Always test in that order.
...
There is two ways to determine the type of a items from the URI only:
 ...
 - bbro and bbrp does not have the lod type of uris(bbr prefix) and shares local id prefix 216.

To determine old style uris one has to make a HTTP call to the item and check the type "manually".
...
def get_full_uri_from_local_id(local_id):
    ...
    type_p = 'raa/bbrp/' + local_id
    type_o = 'raa/bbro' + local_id
    ...
    if local_id.startswith('216'):
        if validate_uri(type_p):
            return type_p

        if validate_uri(type_o):
            return type_o
